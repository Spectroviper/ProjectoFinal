...
class UserChallengeAchievementActivity : ComponentActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        val viewModel = ViewModelProvider(this)[UserChallengeAchievementViewModel::class.java]

        setContent {
            YourAppTheme {
                val user = MyId.user
                val userChallengeAchievementId = intent.getStringExtra("userChallengeAchievementId")
                Log.d("UserChallengeAchievement", "$userChallengeAchievementId")
                val userChallengeAchievement by viewModel.getUserChallengeAchievementByUserChallengeAchievementId(userChallengeAchievementId!!).observeAsState()
                Log.d("UserChallengeAchievement", "$userChallengeAchievement")
                var selectedTab by remember { mutableIntStateOf(0) }
                var option by remember { mutableStateOf("") }
                var showPopup by remember { mutableStateOf(false) }
                userChallengeAchievement?.let {
                    UserChallengeAchievementScreen(
                        userChallengeAchievement = it,
                        selectedTabIndex = 1,
                        onTabSelected = {
                            selectedTab = it
                            if (selectedTab == 0) {
                        ...
                        onFinish = {
                            Log.d("UserChallengeAchievement",user?.userId.toString())
                            Log.d("UserChallengeAchievement",userChallengeAchievementId.toString())
                            viewModel.lockOrUnlockChallengeAchievement(user?.userId!!, userChallengeAchievementId!!) },
