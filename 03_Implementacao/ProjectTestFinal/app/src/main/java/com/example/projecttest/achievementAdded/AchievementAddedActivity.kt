...
class AchievementAddedActivity : ComponentActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        val viewModel = ViewModelProvider(this)[AchievementAddedViewModel::class.java]

        setContent {
            YourAppTheme {
                val user = MyId.user
                val userAchievementId = intent.getStringExtra("userAchievementId")
                val userAchievement by viewModel.getUserAchievementByUserAchievementId(userAchievementId!!).observeAsState()
                var selectedTab by remember { mutableIntStateOf(0) }
                var option by remember { mutableStateOf("") }
                var showPopup by remember { mutableStateOf(false) }

                userAchievement?.let {
                    AchievementAddedScreen(
                        userAchievement = it,
                        selectedTabIndex = 1,
                        onTabSelected = {
                            selectedTab = it
                        ...
                        onFinish = { viewModel.lockOrUnlockAchievement(user?.userId!!, userAchievementId!!) },

                        onRestart = { viewModel.lockOrUnlockAchievement(user?.userId!!, userAchievementId!!) },
